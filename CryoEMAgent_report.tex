...
\begin{tikzpicture}[node distance=0.5cm and 1.5cm, scale=0.95,
  ...
  \node[vlmblock=3.6cm, below=0.4cm of sem1] (sem2)
    {Quality thresholds\\(CTF $<5$\,\AA, $\geq$50 particles/\textmu m)};
...
\begin{tikzpicture}[node distance=0.65cm and 1.8cm, scale=0.93,
  ...
  \draw[arrgrn] (autoappr.south) -- ++(0,-0.4)
    -- ++(-3.0,0) -- ++(0,-10.5) node[]{};  % return arc removed for clarity
...
\begin{tikzpicture}[node distance=0.55cm and 1.8cm, scale=0.92,
  ...
  \draw[arrgrn] (autoappr.south) -- ++(0,-0.3) -- ++(-5.5,0) |- (loop.east);
  \draw[arrgrn] (resum.south)    -- ++(0,-0.3) -- ++(-7.5,0) |- (loop.east);
